...
def CompileInSplitModule(compiler_jar_path, sources, flags=None):
    """Prepares command-line call to Closure Compiler.

    Args:
      compiler_jar_path: Path to the Closure compiler .jar file.
      sources: Source to build, in order.
      flags: A list of additional flags to pass on to Closure Compiler.
...
#            moduleNum += 1
#            args += ['--module', 'module' + str(moduleNum) + ':' + str(moduleCount)]
#            moduleSize = 0
#            moduleCount = 0
        args += ['--js', source.GetPath()]
#        moduleSize += sys.getsizeof(source.GetSource())
#        moduleCount += 1

    if moduleCount:
        args += ['--module', 'module' + str(moduleNum + 1) + ':' + str(moduleCount)]
