...
def _GetModuleJsLine(path, js_source):
    ...
    path = 'http://888.gtimg.com/' + os.path.dirname(path) + "/" + os.path.splitext(os.path.basename(path))[0] + ".amd.js"
...
class _PathSource(source.Source):
    ...
    def writeRequires(self, requiresAll):
        ...
        for moduleName in requiresAll:
            if 'cp_jclq_crossBet_view.js' in self._path and moduleName == 'cp.$':
                print sourceRemoveComment
...
def writeDepsFile(deps, input_namespaces, outputDir, rootWithPrefix):
    out = open(outputDir + "/" + input_namespaces.pop() + ".deps.js", 'w')
...
def genModuleJsEntryPoint(deps, input_namespaces, modulejs, output_dir, root_with_prefix):
    #genernate modulejs entrypoint file
    entryModule = input_namespaces.pop()
    entrypoint_js = entryModule + ".entrypoint.js"
    out = open(output_dir + "/" + entrypoint_js, 'w')
    out.write('// This file was autogenerated by %s.\n' % sys.argv[0])
    out.write('// Please do not edit.\n')
    ...
    out.write('}};\n')
    out.write(open(modulejs, "r").read())
    out.write("\n")
...
def main():
    ...
    options_roots = options.roots
    options_inputs = options.inputs
    options_namespaces = options.namespaces
    ...
    input_namespaces.update(options_namespaces)
    ...
    if output_mode == 'list':
        ...
        genModuleJsEntryPoint(deps, input_namespaces, modulejs, output_dir, root_with_prefix)
    elif output_mode == 'script':
        out.writelines([js_source.GetSource() for js_source in deps])
    elif output_mode == 'compiled':
        input = inputs.pop()
        out = open(os.path.dirname(input) + "/" + os.path.splitext(os.path.basename(input))[0] + ".c.min.js", "w")
...
if __name__ == '__main__':
    main()
    #    print sys.getsizeof(open("E:/workspace/bocai_100/static/v1.0/i/js/jczq/confirm/cp_jczq_confirm_yt.js"),"r")