...
JS_FILE_REGEX = re.compile(r'^.+\.(?:js|soy)$')
HTML_FILE_REGEX = re.compile(r'^.+\.s?html$')
CSS_FILE_REGEX = re.compile(r'^.+\.css$')
PHP_FILE_REGEX = re.compile(r'^.+\.(?:php|tpl)$')
HTML_AND_CSS_FILE_REGEX = re.compile(r'^.+\.(?:s?html|css)$')


def ScanTreeForHtmlFiles(root):
    ...
    Returns:
      An iterable of paths to html files, relative to cwd.
    """
    return ScanTree(root, path_filter=HTML_FILE_REGEX)
...
def ScanTreeForJsFiles(root):
    ...
    """
    return ScanTree(root, path_filter=JS_FILE_REGEX)
